...
//Calculates n Choose k, n a double
...
void simpleMatrix::setSize(int Width, int Height){
    height = Height;
    width = Width;
    matrix.resize(height*width, 0.0);
}

void simpleMatrix::setValue(int x, int y, double value){
    if (y >= height || x >= width){
        exit_error("tried to set "+to_string(x)+"/"+to_string(width)+" "+to_string(y)+"/"+to_string(height)+". Value out of bounds.");
    }
    matrix[x*height + y] = value;
}

double simpleMatrix::getValue(int x, int y){
    if (y >= height || x >= width){
        exit_error("tried to access "+to_string(x)+"/"+to_string(width)+" "+to_string(y)+"/"+to_string(height)+". Value out of bounds\n");
    }
                               ...
                               string variable)
{
    ...
    { 
        exit_error("update_spline_coeffies_JC::This part of memory was not set..."); 
    }
...
// Brent's method
// xacc is relative accuracy
double find_root_brent(std::function<double(double)> f, double x1, double x2, double xacc)
{
    const int find_root_brent_ITMAX=100;
    const double find_root_brent_EPS=1.0e-10;
